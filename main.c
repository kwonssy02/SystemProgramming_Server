...
//	pthread_create(&thread[1], NULL, &keymatrix, (void*)&fd);
	...
	sleep(1);  // 아두이노와 연결 initilizing 시간을 법니다.
		...
		if(strcmp(buf,"0") >= 0){ 
			...
			if(strstr(cmd, "sen") != NULL) {
				...
				*/
				...
				char tlcd_content[14] = "CO";
				tlcd_content[2] = sensorNum[0];
				tlcd_content[3] = ':';
				sprintf(temp, "%d", atoi(cmd2));
				strcat(tlcd_content, temp);

				writeTlcd((sensorNum[0]-'0'), tlcd_content);				
				/*	
				char *argv[3];
				argv[0]= "";
				argv[1]= "r";
				...
				*/
				...
				maxCO = (maxCO >= atoi(temp) ? maxCO : atoi(temp)); // 두 센서중 가장 큰 센서 CO값 저장.
				printf("maxCO: %d\n", maxCO);
				if(maxCO < 400){
				...
				char sendCmd[4] = {'r', '\n', '\0'};
				int ret = write(fd, sendCmd, sizeof(sendCmd)-1);
