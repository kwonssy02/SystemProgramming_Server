...
int tlcd_main(int argc , char **argv)
{
...
int writeTlcd(int nline, char* tempStr) {

	//printf("writeTlcd----------------------------\n");
	//printf("nline: %d,   str: %s\n", nline, tempStr);	
	...
	if(strlen(tempStr) > COLUMN_NUM) {
		strncpy(strWtext, tempStr, COLUMN_NUM);
		strWtext[COLUMN_NUM] = '\0';
	}
	else {
		strcpy(strWtext, tempStr);
	}
...
int main(int argc , char **argv) {
	char tempStr[16] = {'S','T','A','R','T','\0'};
	char tempStr2[16] = {'K','O','M','E','R','A','\0'};
	writeTlcd(1, tempStr);
	writeTlcd(2, tempStr2);
