...
void initScreen(unsigned char *fb_mem )
{
	...
	for(coor_y = START_Y; coor_y < screen_height; coor_y++)
	{
		ptr =   (unsigned long *)fb_mem + screen_width * coor_y ;
		for(coor_x = START_X; coor_x < screen_width; coor_x++)
		{
			*(ptr+coor_x)  =   0x00FFFFFF;
		}
	}
}
*/
...
void fileheader_init(unsigned char *bmp_fileheader, DWORD bfsize)
{
	...
	//
	//*bfType = 'B' | 'M' << 8;               //bfType 'BM'
	memmove(bfType, "BM", 2);

	//*bfSize = 14 + 40 + bfsize;       //bfSize
	memmove(bfSize, &filesize, 4);

	//*bfReserved = 0;                  //bfReserved1  bfReserved2
	memset(bfReserved, 0, 4);

	//*bfOffBits = 54;                  //bfOffBits RGBA  54
...
int SaveCameraToBuffer(unsigned char* buffer) {
	...
	DrawFromRGB565(buffer, m_buffers_preview[cameraindex].virt.p, CAMERA_PREVIEW_WIDTH, CAMERA_PREVIEW_HEIGHT, 640, 480);
